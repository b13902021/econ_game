...
export default function Leaderboard({ gameState }: LeaderboardProps) {
  // 將小隊依照當前公開排名排序，若是 isDead 則強制壓到底部
  const sortedTeams = [...gameState.teams].sort((a, b) => {
    if (a.isDead && !b.isDead) return 1;
    if (!a.isDead && b.isDead) return -1;
    return (a.publicRank || 10) - (b.publicRank || 10);
  });
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody>
              {sortedTeams.map((team, idx) => {
                const currentPublicRank = team.publicRank || 10;
                const previousPublicRank = team.previousRank || 10;
                const rankDiff = team.previousRank === 0 ? 0 : previousPublicRank - currentPublicRank;
                
                // 判斷是否為死亡狀態
                const isSlaughtered = team.isDead;
                ...
                return (
                  <tr 
                    ...
                    className={cn(
                      ...
                      idx === sortedTeams.length - 1 && "border-b-0",
                      idx === 0 && !isSlaughtered ? "bg-amber-50 hover:bg-amber-100" : "hover:bg-zinc-50",
                      isSlaughtered && "bg-red-50 grayscale opacity-80"
                    )}
                  >
                    <td className="p-6 border-r-2 border-black text-center">
                      <div className="flex flex-col items-center justify-center">
                        <span className={cn("text-3xl font-black", idx === 0 && !isSlaughtered ? "text-amber-600" : (isSlaughtered ? "text-red-800" : "text-black"))}>
                          {isSlaughtered ? "☠" : currentPublicRank}
                        </span>
                        {!isSlaughtered && rankDiff > 0 && <span className="text-green-600 text-xs font-bold mt-1">▲ {rankDiff}</span>}
                        {!isSlaughtered && rankDiff < 0 && <span className="text-red-600 text-xs font-bold mt-1">▼ {Math.abs(rankDiff)}</span>}
                        {!isSlaughtered && rankDiff === 0 && team.previousRank !== 0 && <span className="text-zinc-400 text-xs font-bold mt-1">—</span>}
                      </div>
                    </td>

                    <td className="p-6 border-r-2 border-black">
                      <div className="text-2xl font-black tracking-tighter flex items-center gap-2">
                         <span className={isSlaughtered ? "line-through text-red-900" : ""}>{team.name}</span>
                         {isSlaughtered && <span className="text-[10px] bg-red-800 text-white px-2 py-1 uppercase tracking-widest rounded-sm">ELIMINATED</span>}
                      </div>
                    ...
                    <td className="p-6 border-r-2 border-black text-center align-middle">
                      {team.publicJob ? (
                        <span className={cn("inline-block px-4 py-2 font-black text-xs uppercase tracking-widest", isSlaughtered ? "bg-red-900 text-red-200" : "bg-black text-white")}>
                    ...
                    <td className="p-6 text-right">
                      <span className={cn(
                        ...
                        idx === 0 && !isSlaughtered ? "text-amber-600" : (isSlaughtered ? "text-red-800 line-through" : "text-black")
                      )}>
